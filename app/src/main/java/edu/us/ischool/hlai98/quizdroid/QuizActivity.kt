...
class QuizActivity : AppCompatActivity(), OverviewFragment.StartQuiz, QuestionFragment.SeeAnswer, AnswerFragment.NextQuestionOrFinish {
    ...
    override fun StartQuiz() {
        ...
        val ft = fragmentManager.beginTransaction()
        ft.replace(R.id.frameLayout, QuestionFragment.newInstance(current, chosen, questions, questionOption, questionAnswer))
        ft.commit()
        current++
    }
    override fun SeeAnswer(chose: String, answer: String) {
        ...
        val ft = fragmentManager.beginTransaction()
        val fragment: Fragment = AnswerFragment.newInstance(chose, answer, correct.toString(), current.toString(), chosen, totalNumberOfQuestion)
        ft.replace(R.id.frameLayout, fragment)
        ft.commit()
    }
